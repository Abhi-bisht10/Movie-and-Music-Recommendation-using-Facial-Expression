...
def camera():
    i = 0
    colors = {0: (0, 255, 0), 1: (0, 150, 255)}

    try:
        model = tf.keras.models.load_model('model/final_model.h5')
    except TypeError as e:
        print(f"Error loading model: {e}")
        return "Error loading model. Please check the model file or TensorFlow version."
    ...
    while i <= 40:
        ...
        for x, y, w, h in faces:
            ...
            cv2.rectangle(img, (x, y-40), (x+w, y), colors[1], -1)
            cv2.putText(img, predicted_emotion, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
